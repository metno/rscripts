...
# --- Brukervalg ---------------------------------------------------------------
STATION    <- "SN18700"   # Blindern, bytt etter ønske
...
# --- 2. Hent inneværende årets timesdata (PT1H) -------------------------------
...
# Varmeste og kaldeste timeverdi per måned for inneværende år
cur_ext <- cur_df %>%
  group_by(month) %>%
  summarise(
    t_max_val = max(value, na.rm = TRUE),
    t_min_val = min(value, na.rm = TRUE),
    .groups = "drop"
  ) %>%
